...
CI runs `npm run typecheck` + `npm run coverage:ci` + `npm run build` + `npm run check:normalizer-parity`.
`coverage:ci` runs the unit suite (plus the CommonMark conformance baseline) under c8 and enforces the
coverage-baseline ratchet. The DOM-glue and layout specs listed below are maintained by the downstream
host app.
