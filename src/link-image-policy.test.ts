...
// widens via `MarkdownConfig.sanitizeExtension`.
const IMG_EXTENSION = { allowedTags: ['img'], allowedAttr: ['src', 'alt'] }

const APP_ORIGIN = 'https://app.example.com'

// The origin policy is enforced at the sanitizer sink (synchronous), so each test
// scopes its policy around the specific sink/render call with `withConfig`; the
// jsdom setup's DOMPurify backend default is inherited, and the scope restores on
// exit so no per-test teardown is needed.
const withPolicy = <T>(policy: LinkImagePolicy, fn: () => T): T =>
  withConfig({ linkImagePolicy: policy }, fn)

// Sanitize with `<img>` widened into the core allowlist, merged over whatever
// policy scope is already active.
function sanitizeImg(tag: string): string {
  return withConfig({ sanitizeExtension: IMG_EXTENSION }, () => sanitizeRenderedMarkdown(tag))
}

describe('linkImagePolicy — default (no policy)', () => {
  it('leaves rendered links byte-identical to today', () => {
    ...
    // Applying a policy in a scope does not leak into an unscoped render.
    withPolicy(
      {
        allowedLinkPrefixes: [],
...
describe('linkImagePolicy — links', () => {
  it('passes allowed absolute links untouched', () => {
    const html = withPolicy(
      ...
      () =>
        sanitizeRenderedMarkdown('<a href="https://docs.example.com/guide" target="_blank">x</a>'),
    )
  ...
  it('rewrites a blocked link to defaultOrigin and tags it', () => {
    const html = withPolicy(
      ...
      () => sanitizeRenderedMarkdown('<a href="https://evil.com/phish">x</a>'),
    )
  ...
  it('resolves an allowed relative link against defaultOrigin', () => {
    const html = withPolicy(
      ...
      () => sanitizeRenderedMarkdown('<a href="/docs/readme">x</a>'),
    )
  ...
  it('blocks a relative link that resolves outside the allowlist', () => {
    const html = withPolicy(
      ...
      () => sanitizeRenderedMarkdown('<a href="/admin/secret">x</a>'),
    )
    assert.match(html, /class="[^"]*blocked-link/)
  })
  ...
  it('applies to autolinks through the full pipeline', () => {
    const html = withPolicy(
      ...
      () => sanitizeRenderedMarkdown(renderMarkdownUnsafe('see <https://evil.example.com/x>')),
    )
    assert.match(html, /class="[^"]*blocked-link/)
  ...
  it('honors a custom blockedLinkClass', () => {
    const html = withPolicy(
      ...
      () => sanitizeRenderedMarkdown('<a href="https://evil.com/">x</a>'),
    )
    assert.match(html, /class="[^"]*sm-off-origin/)
  })
})

describe('linkImagePolicy — link bypass hardening', () => {
  const policy: LinkImagePolicy = {
  ...
  for (const [name, href] of bypasses) {
    it(`blocks ${name} (${href})`, () => {
      ...
      const html = withPolicy(policy, () =>
        sanitizeRenderedMarkdown(`<a href="${href.replace(/"/g, '&quot;')}">x</a>`),
      )
  ...
  it('does NOT treat good.com@evil.com credentials as a match', () => {
    const html = withPolicy(policy, () =>
      sanitizeRenderedMarkdown('<a href="https://good.com@evil.com/x">x</a>'),
    )
    assert.match(html, /class="[^"]*blocked-link/)
  })
})

describe('linkImagePolicy — images', () => {
  ...
  it('neutralizes a blocked image by stripping src and tagging it', () => {
    const html = withPolicy(
      ...
      () => sanitizeImg('<img src="https://tracker.evil.com/pixel.gif" alt="x">'),
    )
  ...
  it('leaves a src-less (deferred hydration) image alone', () => {
    withPolicy(
      ...
      () => {
        assert.doesNotMatch(sanitizeImg('<img alt="pending">'), /blocked-image/)
  ...
  it('allows data: images by default (allowDataImages defaults true)', () => {
    ...
    const html = withPolicy(
      ...
      () => sanitizeImg(`<img src="${data}" alt="d">`),
    )
  ...
  it('strips base64 data: images when allowDataImages is false', () => {
    const html = withPolicy(
      ...
      () => sanitizeImg('<img src="data:image/png;base64,AAAA" alt="d">'),
    )
    assert.doesNotMatch(html, /src=/)
    assert.match(html, /class="[^"]*blocked-image/)
  })
  ...
  it('resolves an allowed relative image against defaultOrigin', () => {
    const html = withPolicy(
      ...
      () => sanitizeImg('<img src="/img/logo.png" alt="l">'),
    )
  ...
  it('blocks a scheme-relative image src', () => {
    const html = withPolicy(
      ...
      () => sanitizeImg('<img src="//evil.com/x.png" alt="x">'),
    )
    assert.doesNotMatch(html, /src=/)
    assert.match(html, /class="[^"]*blocked-image/)
  })
...
describe('linkImagePolicy — sink + backend coverage', () => {
