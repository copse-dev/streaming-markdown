...
// GFM spec is not published to npm, so its `spec.txt` is vendored into this repo
// under `tests/fixtures/gfm/spec.txt`. Provenance is pinned below and enforced by
// `scripts/fetch-gfm-spec.mts` (SHA-256 tamper check + refresh), the same way the
// reference normalizer is pinned. The example format is identical to CommonMark
// (32-backtick fences), so parsing routes through the shared `parseSpecExamples`.
//
...
/** Pinned upstream provenance for the vendored GFM `spec.txt`. */
export const GFM_SPEC_SOURCE = {
  ...
  /** SHA-256 of the vendored `spec.txt`; `fetch-gfm-spec.mts` fails on any drift. */
...
/**
 * Version of the vendored GFM spec, read from its YAML front matter
...
/** Parse every embedded conformance example from the vendored GFM spec text. */
